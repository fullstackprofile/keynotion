                        ...
                        <div class="row gx-2">
                            ...
                            <div class="col-12 mb-3">
                                ...
                                <textarea class="form-control" id="event-description" name="job_description" rows="6"
                                          required>{{$vacancy['job_description']}}</textarea>
                            </div>
                            {{----------------------------About Role----------------------------------}}
                            <div class="col-12 mb-3 key_topic">
                                <label class="form-label" for="event-description">About The Role<span
                                        style="color: red">*</span></label>
                                @error('about_role')  <div>error {{ $message }}</div> @enderror
                                @foreach($vacancy['about_role'] as $key_number => $key)

                                <div id="inputFormRow">
                                    <label class="form-label">About role items</label>
                                    <input type="text" name="about_role[{{$key_number}}][item]" class="form-control m-input mb-3" placeholder="Enter title" autocomplete="off" value="{{$key['item']}}">
                                ...
                                <button id="addRow" type="button" class="btn btn-info mb-3" data-next-counter="{{count($vacancy['about_role']) + 1}}">Add Item</button>
                            </div>
                            {{----------------------------What we are looking for----------------------------------}}
                            <div class="col-12 mb-3 key_topic">
                                <label class="form-label" for="event-description">What we are looking for<span
                                        style="color: red">*</span></label>
                                @error('looking_for')<div>error {{ $message }}</div> @enderror
                                @foreach($vacancy['looking_for'] as $key_looking => $looking)
                                <div id="inputFormRow_looking">
                                    <label class="form-label">What we are looking for items</label>
                                    <input type="text" name="looking_for[{{$key_looking}}][item]" value="{{$looking['item']}}" class="form-control m-input mb-3" placeholder="Enter title" autocomplete="off">
                                    <div class="input-group-append mb-3">
                                ...
                                <button id="addRow_looking" type="button" class="btn btn-info mb-3" data-next-counter_looking="{{count($vacancy['looking_for']) + 1}}">Add Item</button>
                            </div>
                        </div>
                        {{----------------------------Benefits----------------------------------}}
                        <div class="col-12 mb-3 key_topic">
                            <label class="form-label" for="event-description">Benefits<span
                                    style="color: red">*</span></label>
                            @error('benefits') <div>error {{ $message }}</div> @enderror
                            @foreach($vacancy['benefits'] as $key_benefit => $benefit)

                            <div id="inputFormRow_benefits">
                                <label class="form-label">Benefits items</label>
                                <input type="text" name="benefits[{{$key_benefit}}][item]" value="{{$benefit['item']}}" class="form-control m-input mb-3" placeholder="Enter title" autocomplete="off">
                                <div class="input-group-append mb-3">
                            ...
                            <button id="addRow_benefits" type="button" class="btn btn-info mb-3" data-next-counter_benefits="{{count($vacancy['benefits']) + 1}}">Add Item</button>
                        </div>
                        <div class="col-4">
