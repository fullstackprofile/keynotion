                ...
                <div class="col-12 mb-3">
                    <div class="row justify-content-center justify-content-sm-between">
                        <div class="col-sm-auto text-center">
                            <h5 class="d-inline-block">Brochure Requests</h5>
                        </div>
                        <div  class="col-sm-auto text-center">
                            <div class="search-box" data-list="{&quot;valueNames&quot;:[&quot;title&quot;]}">
                                <form class="position-relative show" data-bs-toggle="search" data-bs-display="static" aria-expanded="true" action="{{route('search_brochure')}}" method="get">
                        ...
                        <table class="table table-hover table-striped overflow-hidden">
                            ...
                            @foreach($brochures as $brochure)
                                <tr>
                                    <td>{{$brochure['id']}}</td>
                                    <td>{{$brochure['name']}}</td>
                                    <td>{{$brochure['surname']}}</td>
                                    <td>{{$brochure['company_name']}}</td>
                                    <td>{{$brochure['job_title']}}</td>
                                    <td>{{$brochure['phone']}}</td>
                                    <td>{{$brochure['corporate_email']}}</td>
                                    <td>{{$brochure['country']}}</td>
                                    <td>{{$brochure['summit_name']}}</td>
                                    <td>{{$brochure['comment']}}</td>
                                    <td>{{$brochure['learn']}}</td>
                                </tr>
                            @endforeach
                            </tbody>
                        </table>
